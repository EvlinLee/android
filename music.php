...
//搜索语句
$sql = "SELECT * FROM `#@__vod` WHERE `vod_class`=5 AND `vod_delstate`=0 AND `vod_enable`=1 order by vod_id desc";
...
//获得缓存的剧集
$movies = get_mem('music_'.$pageCurrent);
if(empty($movies)){
	$movies = $dosql->GetPageInfo($sql,$page_size,$pageCurrent);
	if(!empty($movies)){
		set_mem('music_'.$pageCurrent,$movies);
	}
}
