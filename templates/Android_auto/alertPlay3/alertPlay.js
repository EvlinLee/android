...
function getUrls(u,iid) {
    ...
    window.MERGE_URL = window.PLAY_URL = 'http://10.65.255.9:8080/app/reveal/?u=';

    var api = (window.PLAY_URL && window.PLAY_URL != '') ? window.PLAY_URL : 'http://10.65.255.9:8080/app/reveal/?u=' ;
    var mapi = (window.MERGE_URL && window.MERGE_URL != '') ? window.MERGE_URL : 'http://10.65.255.9:8080/app/reveal/?u=' ;
    var ottApi = (window.OTT_API && window.OTT_API != '') ? window.OTT_API : false ;
    
    var hls,ottn,otth;
    var params = encodeURIComponent(u)+'&iid='+iid;
    //hls = api+params+'&quality=1&m3u8_location=http%3A%2F%2Frt.vsea.tv%2F%7Cflags%7C%2F%7Cid%7C.m3u8';
    hls = api+params+'&quality=1&m3u8_location=http%3A%2F%2F10.65.255.9%2F%7Cflags%7C%2F%7Cid%7C.m3u8';
...
function tanPlay(url){
	url = encodeURIComponent(url);
	var ipUrl = "http://42.121.58.216:8090/need/ip/02222601005592002433597811/"+url;
	var ipqamUrl = "http://42.121.58.216:8090/need/ipqam/02222601005592002433597811/"+url;
	$(".ip_url a").attr('href',ipUrl);
	$(".ipqam_url a").attr('href',ipqamUrl);
	$(".tanbg").show();
	$(".tan").show();
}
...
$(function(){
	var str = '<div class="tanbg" onClick="closePlay();"></div><div class="tan"><div class="ip_url"><a href="">IP</a></div><div class="ipqam_url"><a href="">IPQAM</a></div><div><a href="javascript:void();" onClick="closePlay();">取消</a></div></div>';
