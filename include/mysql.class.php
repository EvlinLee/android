<?php	if(!defined('SHIHAI')) exit('Request Error!');
...
class MySql
{
    ...
    */
    ...
    //分页获取信息
	function GetPageInfo($sql,$page_size,$pageCurrent=1){
		$start = $page_size*($pageCurrent-1);
		$sql .= " LIMIT ".$start.",".$page_size;
		$row = $this->Execute($sql);
		return $this->GetAllArray();
	}
	
	//获取所有结果集
	function GetAllArray($id='me'){
		$result = array();
		while($row = $this->GetArray($id)){
			$result[] = $row;
		}
		return $result;
	}
	...
	function Update( $table,$dataArray,$condition="") {
		...
		if(!$this->ExecNoneQuery($sql)) return false;
		return true;
	}
	//插入数据
    ...
    function GetAll($sql='',$acctype=MYSQL_ASSOC)
    {
        ...
        $this->Execute($sql,'all');
        
        $res = array();
        while($row = $this->GetArray('all', $acctype)){
        	$res[] = $row;
        }
        if(!is_array($res))
        {
